		...
		methods = {
				loadOptions: {
						async getTenantUri(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]>{
								...
								const tenantURI = await portApiRequest.call(this, 'GET', 'accounts/my');
								returnData.push({
				...
				for (let i = 0; i < length; i++) {
						if (resource === 'member') {
								if (operation === 'getAll') {                
										...
										const totalMembers = responseData['count'];
										...
										const pagesToFetch = Math.ceil(numberEntries/pageSize);
										const lastPageSize = numberEntries % pageSize;
								...
								if (operation === 'search') {
										...
										const payload = searchMemberPayload(tenantUri, dataSource, memberName);
										responseData = await portApiRequest.call(this, 'POST', 'search/member', payload);
								}
								if(operation === 'create'){                    
										const memberName = this.getNodeParameter('memberName', i) as string;
										const additionalFields = this.getNodeParameter('additionalFields', i) as IDataObject;
										const payload = createMemberPayload(memberName, additionalFields);
						...
						if (resource === 'interaction') {
								if (operation === 'create') {                    
										...
										const payload = createInteractionPayload(interactionType, interactionDescription, scoreName, scoreValue, scoreSize, 'interaction');
